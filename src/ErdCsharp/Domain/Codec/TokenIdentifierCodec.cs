...
namespace ErdCsharp.Domain.Codec
{
    public class TokenIdentifierCodec : IBinaryCodec
    {
        ...
        public (IBinaryType Value, int BytesLength) DecodeNested(byte[] data, TypeValue type)
        {
            ...
            return (ESDTIdentifierValue.From(value.ValueOf<BytesValue>().Buffer), bytesLength);
        }

        public IBinaryType DecodeTopLevel(byte[] data, TypeValue type)
        {
            var bytesValue = _bytesBinaryCodec.DecodeTopLevel(data, type);
            return ESDTIdentifierValue.From(bytesValue.ValueOf<BytesValue>().Buffer);
        }
        ...
        private static ESDTIdentifierValue Get(IBinaryType value)
        {
            if (value is ESDTIdentifierValue tokenIdentifierValue)
            {
